...
class MenuElement extends React.Component {
    ...
    render() { 
       ...
       const checkedstyle = {height: "210px", width: "210px", backgroundColor: "white", border: "4px solid lightgreen"};
       ...
       const curStPad = this.props.orders.map(order=>order.key).filter(k=>k===this.props.pkey).length > 0 ? checkedstylePad : uncheckedstylePad
        return (
            <div className={'transform'} style = {curStPad}>
                <Card outlined boxShadow="0 15px 50px -12px rgba(100,200,0,0.8)" raised={this.props.orders.map(order=>order.key).filter(k=>k===this.props.pkey).length > 0 ? true : false} onClick={this.checkkey.bind(this, key)} >
                ...
                <Box style={{float:"left", paddingTop:"10px", color: "#767676"}}>QTY&nbsp;{quanitity}</Box>
            </div>
        );
    }
