...
import { useState } from 'react';
 ...
 function SummaryOrders(props){
    ...
    const elements = redOrd.map(element => <div> <br></br><div style={{minWidth:"100%"}}>
            <div style ={{float: "left", width: "100%", color: "#767676"}}>
                <div style ={{float: "left", paddingLeft: "20px",fontWeight: "bold", overflowWrap: "break-word"}}>
                    {element.order.name +" "+(element.order.options.length>0?(element.order.options.reduce((prev,curr)=>prev.elements.find(e=>e.isChosen).name+" "+curr.elements.find(e=>e.isChosen).name)):"")+ (element.qty>1? (" x " + element.qty):"")}
                </div>
